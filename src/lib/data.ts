import { MenuItem, Order, Table, User, Transaction } from './types';
...
export const generateStaffUsers = (): User[] => {
  return [
...
export const calculateOrderTime = (order: Order): number => {
  const now = Date.now();
  return Math.floor((now - order.createdAt) / 1000 / 60);
};

export const getOrderUrgency = (order: Order): 'normal' | 'warning' | 'urgent' => {
